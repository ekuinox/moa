...
vi.mock("./bindings.generated", () => ({
  commands: {
    ...
    createFiscalYear: vi.fn(async (input) => ({ id: "tauri-fiscal-year", ...input })),
    createPartner: vi.fn(async (input) => ({ id: "tauri-partner", ...input })),
    deleteCategory: vi.fn(async () => undefined),
    deleteFiscalYear: vi.fn(async () => undefined),
    deletePartner: vi.fn(async () => undefined),
    generateFiscalYear: vi.fn(async (input) => ({
      id: "tauri-fiscal-year",
      name: `${input.startYear}年度`,
      startMonth: `${input.startYear}-04`,
      endMonth: `${input.startYear + 1}-03`,
    })),
    getFiscalYearSetting: vi.fn(async () => ({
      startMonth: 4,
      durationMonths: 12,
      namingRule: "start_year",
    })),
    listCategories: vi.fn(async () => []),
    listFiscalYears: vi.fn(async () => []),
    listPartners: vi.fn(async () => []),
    saveFiscalYearSetting: vi.fn(async (input) => input),
    updateCategory: vi.fn(async (input) => input),
    updateFiscalYear: vi.fn(async (input) => input),
