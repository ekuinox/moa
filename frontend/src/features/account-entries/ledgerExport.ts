...
/** CSV/印刷に出す列の定義。key は LedgerExportRow の値を取り出すために使い、配列順が出力順になる。 */
export interface LedgerExportColumn {
  /** 出力行オブジェクト上のフィールド名。列の並び替えや取引先列の有無をここで吸収する。 */
  readonly key: "occurredOn" | "partner" | "categories" | "description" | "amount";
  /** CSV ヘッダーおよび印刷表ヘッダーとして出力する表示名。 */
  readonly label: string;
}

/** 表示中の明細 1 行を出力向けに整形した値。画面表示用の React 部品には依存させない。 */
export interface LedgerExportRow {
  /** 元の明細 ID。CSV の列には出さず、印刷 DOM の安定した行 key として使う。 */
  readonly entryId: string;
  /** 画面の読み取り表示と同じ M/D 形式の日付。 */
  readonly occurredOn: string;
  /** すべての取引先表示のときだけ出力する取引先名。取引先を選択中の表では列ごと省略する。 */
  readonly partner: string | undefined;
  /** 複数種別を 1 セルに収めるため、表示名を区切って連結した値。 */
  readonly categories: string;
  /** 摘要はユーザー入力値をそのまま保持し、CSV 側で必要に応じてエスケープする。 */
  readonly description: string;
  /** 表計算で数値として扱えるよう、通貨記号や桁区切りを含めない整数文字列。 */
  readonly amount: string;
}

/** CSV/印刷に必要な列、行、ファイル名、見出しをまとめた出力スナップショット。 */
export interface LedgerExportScope {
  /** 出力する列定義。取引先列の有無など「表示中の表」の形をここに閉じ込める。 */
  readonly columns: readonly LedgerExportColumn[];
  /** Tauri 保存ダイアログの既定ファイル名。現在は CSV 保存で使う。 */
  readonly fileName: string;
  /** 買掛/売掛の種別。ファイル名や帳票種別判定に使う。 */
  readonly kind: AccountEntryKind;
  /** 選択中の取引先表示名。ファイル名や印刷見出しで使う。 */
  readonly partnerLabel: string;
  /** 選択中期間の表示名。ファイル名や印刷見出しで使う。 */
  readonly periodLabel: string;
  /** 出力する明細行。createLedgerExportScope に渡された visibleEntries と同じ順序を保つ。 */
  readonly rows: readonly LedgerExportRow[];
  /** 買掛表/売掛表などの帳票タイトル。印刷見出しでも再利用できるよう保持する。 */
  readonly title: string;
  /** 合計行の金額。明細の amount と同じく数値リテラルとして出力する。 */
  readonly totalAmount: string;
}

/** 現在の台帳表示状態から CSV/印刷用スナップショットを作るための入力。 */
export interface CreateLedgerExportScopeInput {
  /** 取引先が選択され編集可能な表かどうか。false のとき出力に取引先列を出す。 */
  ...
  readonly kind: AccountEntryKind;
  /** 取引先 ID を表示名へ変換するための Map。すべての取引先表示の出力で使う。 */
  ...
  /** YYYY-MM または FYxxxx の期間キー。ファイル名や印刷見出しの期間表示へ変換する。 */
  readonly selectedPeriod: string;
  /** 画面タイトル。印刷見出しなど、出力スナップショットの文脈として保持する。 */
  readonly title: string;
